...
buf.push('>\n      <div');
buf.push(attrs({ terse: true, 'id':('organisation_new_name') }, {}));
buf.push('>\n        <label>Name</label>\n        <input');
buf.push(attrs({ terse: true, 'name':("name"), 'value':(organisation.name) }, {"name":true,"value":true}));
buf.push('>');
if ( errors.name)
{
buf.push('<span');
buf.push(attrs({ terse: true, "class": ('error') }, {}));
buf.push('>' + escape((interp = errors.name.type) == null ? '' : interp) + '</span>');
}
buf.push('\n      </div>');
if ( locals.require_slug)
{
buf.push('\n      <p');
buf.push(attrs({ terse: true, 'id':('organisation_new_slug') }, {}));
buf.push('>\n        <label>Slug</label>\n        <input');
buf.push(attrs({ terse: true, 'name':("slug"), 'value':(organisation.slug) }, {"name":true,"value":true}));
buf.push('><span');
buf.push(attrs({ terse: true, "class": ('help') }, {}));
buf.push('> Can\'t create one automatically from the name</span>');
if ( errors.slug)
{
buf.push('<span');
buf.push(attrs({ terse: true, "class": ('error') }, {}));
buf.push('>' + escape((interp = errors.slug.type) == null ? '' : interp) + '</span>');
}
buf.push('\n      </p>');
}
buf.push('\n      <input');
buf.push(attrs({ terse: true, 'type':("submit"), 'value':("Create new organisation"), "class": ("btn") }, {"type":true,"class":true,"value":true}));
buf.push('>\n    </form>\n  </div>\n  <footer');
