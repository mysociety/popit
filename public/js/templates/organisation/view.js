...
buf.push('>\n  <title>' + escape((interp =  organisation.name ) == null ? '' : interp) + '</title>\n  <link');
...
buf.push('>\n    <style>\n      a.hover_edit {\n        float: right;\n        color: #ddd;\n        font-size: 80%;\n      }    \n      \n      a.hover_edit:hover {\n        color: #333;\n      }\n      \n      \n    </style>\n    <article');
buf.push(attrs({ terse: true, 'id':("person-james_gordon_brown"), 'itemscope':(true), 'itemtype':("http://schema.org/Organisation"), "class": ("person") }, {"class":true,"id":true,"itemscope":true,"itemtype":true}));
buf.push('>\n      <header');
buf.push(attrs({ terse: true, 'id':("person-" + (organisation.slug) + ""), "class": ('person-header-nav') }, {"id":true}));
buf.push('>\n        <nav><a');
buf.push(attrs({ terse: true, 'href':("#FIXME-previous-person"), 'title':("Previous Organisation: Organisation Name"), "class": ('previous-person') }, {"href":true,"title":true}));
buf.push('><em>Previous&nbsp;Organisation:</em><strong>Organisation&nbsp;Name</strong></a><a');
buf.push(attrs({ terse: true, 'href':("#FIXME-next-person"), 'title':("Next Organisation: Organisation Name"), "class": ('next-person') }, {"href":true,"title":true}));
buf.push('><em>Next&nbsp;Organisation:</em><strong>Organisation&nbsp;Name</strong></a></nav>\n        <h1');
buf.push(attrs({ terse: true, 'title':("" + ( organisation.name ) + ""), "class": ('current-person') }, {"title":true}));
buf.push('>' + escape((interp =  organisation.name ) == null ? '' : interp) + '</h1>\n        <div');
buf.push(attrs({ terse: true, "class": ('find-person') }, {}));
buf.push('>\n          <label');
buf.push(attrs({ terse: true, 'for':("person-search") }, {"for":true}));
buf.push('>Search for Organisation</label>\n          <input');
...
buf.push('>\n        <h2>Raw data</h2>\n        <p>The details for this organisation are also available in <a');
buf.push(attrs({ terse: true, 'href':("/api/v1/organisation/" + (organisation.id) + "") }, {"href":true}));
buf.push('>JSON</a>.</p>\n      </section>\n    </article>\n  </div>\n  <footer');
