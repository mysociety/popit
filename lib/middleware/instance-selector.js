...
module.exports = function instanceSelector () {
    ...
    var abandonedInstances = [
        'kenyan-politicians'
    ];

    var siteAbandoned = function(res) {
        res.status(410).render('errors/site_abandoned.html');
    };

    var site_not_found = function (res) {
        // If no site found then render the no site 404 page
        res.status(404).render( 'errors/no_site_found.html' );
    };
    ...
    return function select_instance(req, res, next){
        ...
        if (abandonedInstances.indexOf(slug) != -1) {
            return siteAbandoned(res);
        }
        ...
        master.model('Instance').findOne(
            ...
            function (err, instance) {
                ...
                if ( instance ) {
                    ...
                    popit.set_instance_id( instance._id );
                    // if there is no dbname it falls back to the instance name
                    if ( instance.dbname ) {
