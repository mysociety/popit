...
// Add in the partial dates
PersonSchema.plugin( partialDatePlugin, {fieldName: 'personal_details.date_of_birth'} );
PersonSchema.plugin( partialDatePlugin, {fieldName: 'personal_details.date_of_death'} );
...
PersonSchema
  .virtual('api_base_url')
  .get( function () { return config.api_root_url + '/person'; } );

PersonSchema
  .virtual('api_url')
  .get( function () { return this.api_base_url + '/' + this.id; } );
...
PersonSchema.methods.additional_meta = function() {
  return {
    positions_api_url: 'position?person=' + this.id,
    api_url:           'person/' + this.id,
  };
};
