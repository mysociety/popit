...
OrganisationSchema.methods.find_positions = function(cb) {
  return this
    .model('Position')
    .find( { organisation: this.id }, cb );
};

OrganisationSchema.methods.additional_meta = function() {
  return {
    positions_api_url: 'position?organisation=' + this.id,
    api_url:           'organisation/' + this.id,
  };
};
