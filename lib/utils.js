var check      = require('validator').check,
    ...
    // FIXME - should use proper npm package once pull request accepted
...
module.exports.load_test_fixtures = function (load_test_fixtures_done) {
    ...
    var instance = fixtures.connect( config.MongoDB.popit_prefix + 'foobar' );

    master.load( '../fixtures/master.js', function () {
        instance.load( '../fixtures/foobar_instance.js', function () {
            master.db.close();
            instance.db.close();
...
module.exports.checkDatabaseConnection = function () {
  ...
  var errorMessage = function(err) {
    console.log("\033[31mCan't connect to MongoDB server on "+host+" - is it running?\033[0m \n" + err);
  }
...
module.exports.image_proxy_helper = function(image_object, height, width) {

  var Validator = require('validator').Validator;
  var v = new Validator();

  v.error = function(msg) {
      console.log('Image Proxy: Invalid Parameters');
  }
  ...
  if(image_object.url)
        return config.image_proxy_server.base_url + "?url=" +  encodeURI(image_object.url) + options;
      else
        return config.image_proxy_server.base_url + "?url=" +  encodeURI(image_object.url) + options;
};