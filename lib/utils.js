...
function translateObject(obj, langs, defaultLang) {
  ...
  if (_.isArray(obj)) {
    obj = obj.map( function( item ) {
      ...
      return translateObject(item, langs, defaultLang);
    });
  }
  ...
  if (keys.every(isValidLanguage)) {
    ...
    keys.forEach(function(key) {
      new_obj[key] = dothetranslations(obj[key], langs, defaultLang);
    });
    return new_obj;
  }
}
  ...
  doc.get = function(path, type) {
    ...
    if ( _.isArray(res) ) {
      res = res.map(function(entry) {
        if ( _.isObject(entry) ) {
          return translateObject(entry, [lang], defaultLang);
        } else {
          return dothetranslations(entry, [lang], defaultLang);
        }
      });
    } else {
...
function getLangName(lang) {
  lang = lang + ''; // make sure it's a string
  var langTag = tags.language(lang);
  // if the language is of the form lg-lglg then we need
  // to strip the -lglg to get the name of it as langtags
  // doesn't provide names for subtypes for some reason.
  if ( !langTag ) {
    var shortLang = lang.substring(0,lang.indexOf('-'));
    langTag = tags.language(shortLang);
  }
  if ( langTag ) {
    var descs = langTag.descriptions();
...
module.exports.getLangName = getLangName;

/*
 * we only allow the language to be the default language or the alt language and we don't allow
 * it to be one we don't know about as then the saving/loading code doesn't work
 */
function getLangs(req) {
  ...
  if ( ( lang != defaultLang && lang != altLang ) || !isValidLanguage(lang) ) {
    lang = defaultLang;
  }
