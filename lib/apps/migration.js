...
module.exports = function () {
  ...
  app.doImport = function(popit, schema, mappings, data, cb) {
    ...
    var Person = popit.model('Person'),
        ...
        person, positions = [], people = [], ogranizations = [],
        useable, res = [];

    // async.forEach does not support objects, data is an object, not an array
    async.forEach(Object.keys(data), function(attributes, callback) {
      ...
      async.parallel([
          ...
          function(callback){
            /*
            async.forEach(useable['position'], function(value, key) {
      ...
      async.forEachSeries(_.union(people, positions, ogranizations), function(item, callback) {
        item.save(function (err) {
          callback(err);
        });
      }, function(err){
        cb(err, people, positions, ogranizations);
      });
  ...
  app.mounted(function(parent) {
     ...
     */
    var mappingGet = function (req, res, next, id) {
