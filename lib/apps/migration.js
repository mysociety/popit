...
var express            = require('../express-inherit'),
    ...
    fs                 = require('fs'),
    requirePermission  = require('../apps/auth').requirePermission,
  ...
  */
  ...
  app.doImport = function(popit, schema, mappings, data, updateCb, functionCallback) {
    ...
    var Person = popit.model('Person'),
      ...
      Membership = popit.model('Membership'),
      Organization = popit.model('Organization'),
      ContactDetail = popit.model('ContactDetail'),
      memberships = [], people = [], organizations = [];
    ...
    async.forEachSeries(Object.keys(data), function(key, forEachCallback) {
      ...
      async.parallel([
        ...
        function(callback) {
          _.each(useable.contact, function(value, key) {
            if (value) {
              var cd = new ContactDetail({type: key, value: value});
        ...
        function(callback) {
          _.each(useable.links, function(value, key) {
            if (value) {
              var l = new Link({note: key, url: value});
              person.links.push(l); 
            }
          });
          callback();
        ...
        function(callback) {
          if (useable.membership) {
            async.forEach(Object.keys(useable.membership), function(key, cb) {
              var value = useable.membership[key];
              if (value) {
                Organization.findOne({name: value}, function(err, doc) {
                  if (err) winston.error( err );
                  else {
                    // find organization in array to avoid problems when saving
                    doc = doc || _.find(organizations, function(e) {return e.name == value;});
                    if (!doc) {
                      doc = new Organization({name: value});
                      organizations.push(doc);
                    }
                    var pos = new Membership({
                      role: key,
                      organization_id: doc,
                      person_id: person,
                    });
                    memberships.push(pos);
                  }
                  cb(err);
                }); 
              } else {
        ...
        function(err, results) {
          ...
          winston.verbose('memberships:', memberships);
          winston.verbose('organization:', organizations);
      ...
      var toBeSaved = _.union(people, memberships, organizations);
      var total = toBeSaved.length;
      var progress = 0;

      // save people and memberships to database
      ...
      async.forEachSeries(toBeSaved, function(item, callback) {
        ...
        reIndex(config.MongoDB.popit_prefix + popit.instance_name(), function(err) {
          functionCallback(err, people, memberships, organizations);
        });
      });
   ...
   * Shows the migration/ index page
   */
  app.get('/', requirePermission('can_edit_instance'), function (req,res) {
   ...
   */
  app.get('/clear', requirePermission('can_edit_instance'), function (req,res) {
  ...
  app.get('/progress/:id', requirePermission('can_edit_instance'), function (req, res, next) {
    req.popit.model('Migration').findOne({_id: req.params.id}, function(err, doc) {
      if (err) winston.error( err );
      if (!doc) {
        next( new Error404() );
  ...
  app.get('/status/:id', requirePermission('can_edit_instance'), statusGet);
  
  app.post('/upload', requirePermission('can_edit_instance'), uploadPost);
  app.get('/upload', requirePermission('can_edit_instance'), uploadGet);
