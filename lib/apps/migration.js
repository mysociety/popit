...
module.exports = function () {
  ...
  */
  ...
  app.doImport = function(popit, schema, mappings, data, updateCb, functionCallback) {
    ...
    async.forEach(Object.keys(data), function(key, forEachCallback) {
      ...
      async.forEach(toBeSaved, function(item, callback) {
   ...
   * Shows the migration/ index page
   */
  app.get('/', user.can('edit instance'), function (req,res) {
   ...
   */
  app.get('/clear', user.can('edit instance'), function (req,res) {
  ...
  app.get('/progress/:id', user.can('edit instance'), function (req, res, next) {
    req.popit.model('Migration').findOne({_id: req.params.id}, function(err, doc) {
      if (err) winston.error( err );
      if (!doc) {
        next( new Error404() );
  ...
  app.get('/status/:id', user.can('edit instance'), statusGet);
  
  app.post('/upload', user.can('edit instance'), uploadPost);
  app.get('/upload', user.can('edit instance'), uploadGet);
