...
module.exports = function () {
  ...
  */
   ...
   * Shows the migration/ index page
   */
  app.get('/', requireUserOrGuest, function (req,res) {
   ...
   */
  app.get('/clear', requireUserOrGuest, function (req,res) {
  ...
  app.get('/progress/:id', requireUserOrGuest, function (req, res, next) {
    req.popit.model('Migration').findOne({_id: req.params.id}, function(err, doc) {
      if (err) winston.error( err );
      if (!doc) {
        next( new Error404() );
  ...
  app.get('/status/:id', requireUserOrGuest, statusGet);
  
  app.post('/upload', requireUserOrGuest, uploadPost);
  app.get('/upload', requireUserOrGuest, uploadGet);
