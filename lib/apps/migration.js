...
module.exports = function () {
  ...
  app.schemas = {'person': {
      ...
      options: {
        ...
        contact: {
          ...
          strict: false,
          suggestions: ['Telephone', 'Fax', 'Email', 'Skype']
        },
        links: {
          title: 'Link',
          model: {
  ...
  app.doImport = function(popit, schema, mappings, data, cb) {
    ...
    var PersonModel = popit.model('Person'),
        LinkModel = popit.model('Link'),
        ContactDetailModel = popit.model('ContactDetail'),
        person, people = [], count = 0,
        useable, res = [];
    ...
    _.each(data, function(attributes) {
      ...
      person = new PersonModel({'name': useable.name});
    ...
    async.forEachSeries(people, function(item, callback) {
