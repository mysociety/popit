...
module.exports = function (opts) {
  ...
  function save_as_multilingual(body, obj, lang, defaultLang) {
    ...
    _.keys(body).forEach(function(field) {
      ...
      } else if ( [ 'id', '_id', 'memberships', 'posts', 'images', 'birth_date', 'death_date', 'start_date', 'end_date', 'founding_date', 'dissolution_date' ].indexOf(field) == -1 ) {
        if ( _.isObject(item) ) {
          item[lang] = body[field];
        } else if ( lang != defaultLang ) {
          var translated = {};
          translated[defaultLang] = item;
  ...
  app.put('/:id(*)', function(req, res, next) {
    ...
    req.body = save_as_multilingual(req.body, req.object, lang, req.popit.setting('language'));
    next();
  });
  ...
  app.get('/', function (req,res,next) {
    ...
    if (search) {
      ...
      query.exec(function(err, docs) {
        if (err) throw err;
        res.locals[opts.template_objects_name] = docs.map(function(d) { return translationDecorator(d, req); });
  ...
  app.param('id', function loadDocumentById (req, res, next, id) {
    ...
    res
      ...
      .exec(function(err, memberships) {
        memberships = memberships.map(function(m) { return translationDecorator(m, req); } );
