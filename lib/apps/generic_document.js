...
module.exports = function (opts) {
  ...
  // Remove blank values from field_sources object
  app.put('/:id(*)', function(req, res, next) {
    function removeBlanks(obj) {
      var ret = {};
      _.each(obj, function(val, key) {
        if (_.isArray(val)) {
          var filtered = _.compact(val);
          if (filtered.length > 0) {
            ret[key] = filtered;
          }
        } else if (_.isObject(val)) {
          ret[key] = removeBlanks(val);
        } else {
          if (val) {
            ret[key] = val;
          }
    ...
    req.body.field_sources = removeBlanks(req.body.field_sources);
    next();
  });
