...
module.exports = function () {
  ...
  app.mounted(function(parent){
    ...
    app.param('contactId', function loadContact (req, res, next, contact_id) {
      
      var person = res.local('person');
      var contact_details = person.contact_details;
      ...
      if ( contact_id === 'new') {
        ...
        contact = contact_details[ contact_details.length - 1 ];
      } else {
        contact = person.contact_details.id(contact_id);
      };

      if ( !contact) return next( new Error404() );

      res.local('contact', contact );
