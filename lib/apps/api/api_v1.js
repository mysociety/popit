...
function create_api_endpoint ( options ) {
  ...
  api_endpoint.param('document_id', function (req, res, next, document_id) {
    ...
    query.exec( function (err, doc) {
      ...
      if ( document_id != doc.id ) {
        res.redirect(base_url(req) + req.originalUrl + '/' + doc.id );
      } else {
        res.locals.doc = doc;
        return next();
      }
  ...
  function read_collection (req,res,next) {
    ...
    var api_base_url  = base_url(req) + req.originalUrl + '/';
    ...
    _.keys(result).forEach( function (key) {
      if (_.isArray(result[key])) {
        result[key].forEach( function (item) {
          ...
          // TODO improve this hack - we want to add a url to the meta so that thi image
          // can be retrieved. And we want to indicatu if the image proxy can be used with
          // this url.
          if ( key == 'images' ) {
            if ( item.url ) {
  ...
  function create_in_subcollection (req,res) {
    ...
    doc.save(function(err) {
      if (err) {
        ...
        res.header(
          'location',
          base_url(req) + req.originalUrl + '/' + doc.id + '/' + sub_name + '/' + created_doc_id
        );
        res.jsonp({
          ok: true,
