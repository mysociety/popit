...
exports.get = function( req, res, next ) {
    ...
    async.detect(req.object.images, function(item, callback) {
        ...
        if (!image) {
          var Image = req.popit.model('Image');
          var guessedPath = req.popit.files_dir(Image.path_base(image_id));
          fs.exists(guessedPath, function(exists) {
            if (!exists) {
              return next(new Error404());
            }
            Image.mime_type_for(guessedPath, function(err, mime) {
              if (err) {
                return next(err);
              }
              res.set('Content-Type', mime);
              res.sendfile(guessedPath);
            });
          });
          return;
        }
