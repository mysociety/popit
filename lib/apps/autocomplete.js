...
var autocomplete_model = function(model, attr, modelQueryCallback) {
  modelQueryCallback = modelQueryCallback || function(_, done) { done(null, {}); };
  return function(req, res, next) {
    var langs = getLangs(req);
    modelQueryCallback(req, function(err, q) {
      if (err) {
        return next(err);
      }
      var term = req.param(attr),
          query = req.popit
            .model(model)
            .find(q, 'id name label slug')
            .limit(10);
      if (term) {
        var re = new RegExp( regexp_quote(term), 'i');
        var term_obj = {};
        term_obj[attr] = { $regex: re };
        var terms = [term_obj];

        var lang_attr = attr + '.' + langs.lang;

        term_obj = {};
        term_obj[lang_attr] = { $regex: re };
        terms.push(term_obj);

        if ( langs.lang != langs.defaultLang ) {
          lang_attr = attr + '.' + langs.defaultLang;
          term_obj = {};
          term_obj[lang_attr] = { $regex: re };
          terms.push(term_obj);
        }
      ...
      query.exec(function(err,docs) {
        ...
        docs = docs.map(function (doc) { return translationDecorator(doc, req); });
...
app.get('/organizations', autocomplete_model('Organization', 'name', function(req, done) {
  if (!req.query.post_id) {
    return done(null, {});
  }
  req.popit.model('Post').findById(req.query.post_id, function(err, post) {
    if (err) {
      return done(err);
    }
    done(null, {_id: post.organization_id});
  });
}) );
...
app.get('/classifications', autocomplete_results( 'Organization', 'classification' ) );
