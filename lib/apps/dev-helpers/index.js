...
module.exports = function () {
  ...
  app.post('/add_instance_to_master', function(req,res) {
  ...
  app.post('/delete_this_instance_database', function(req,res) {
    var db = req.popit.instance_db().db;
    
    db.dropDatabase(function(err) {
        if (err) throw err;
        db.close();
        res.send('OK - database deleted');
    });
  });
  
  // Load the testing fixture
  app.post('/load_testing_fixture', function (req,res) {
    ...
    instance.load( '../fixtures/foobar_instance.js', function (err) {
      if(err) throw err;
      instance.db.close();
      res.send('OK - test fixtures loaded');            
    });
