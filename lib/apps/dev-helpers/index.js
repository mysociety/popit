...
winston.warn(
    "\n" +
    "**************************************************************\n" +
    "* WARNING - the dev helpers have been activated. This means  *\n" +
    "* that anyone going to '/_dev' will be able to delete        *\n" +
    "* instances. If this is not a dev or test server you should  *\n" +
    "* change NODE_ENV to 'production' or similar.                *\n" +
    "**************************************************************\n"
);
...
module.exports = function () {
  ...
  app.post('/delete_instance', function(req,res,next) {
    ...
    req.popit.model('Instance').findOne({slug: instance_slug}).remove(function(err) {
      ...
      dropInstanceDatabase(
        instance_slug,
        function(err) {
          res.locals.message = "OK - deleted instance '" + instance_slug + "'";
  ...
  app.post('/delete_instance_database', function(req,res,next) {
    dropInstanceDatabase(
      ...
      function(err) {
        res.locals.message = 'OK - database deleted';
        req.popit.clear_cached_settings();
  ...
  app.post('/load_test_fixture', function (req,res,next) {
    ...
    instance.load( '../fixtures/test_instance.js', function (err) {
      if (err) return next(err);
      res.locals.message = 'OK - test fixture loaded';            
  ...
  app.post('/fetch_all_active_instance_info', function ( req, res, next ) {
    req.popit.model('Instance').fetch_all_active_instance_info( function (err) {
      res.locals.message = 'OK - all instances synced';            
      next(err);
    });
  });
  ...
  app.get( '/log_messages', function (req, res, next) {
    ...
    res.locals.message = 'OK - messages sent to logs';            

    show_index_page(req,res);
  });
  
  
  app.post('/change_guest_access', function (req, res, next) {
    var do_what = req.param("do_what");
    if ( ! do_what ) {
      return next( new Error('must provide a "do_what" value') );      
    }
    
    // If we have enable set flag true, otherwise assume false
    var allow_guests = do_what == 'enable';

    var msg = allow_guests ? "Guest access enabled" : "Guest access disabled";
    
    req.popit.set_setting('allow_guest_access', allow_guests, function (err) {
