...
var express                 = require('../express-inherit'),
    ...
    popit                   = require('../popit'),
    Error401                = require('../errors').Error401,
...
module.exports.requireUser = function requireUser(req, res, next) {
  ...
  req.session.post_login_redirect_to = req.current_absolute_url();
  res.redirect('/login');
};
...
module.exports.apiRequireUser = function apiRequireUser (req, res, next) {  
  ...
  passport.authenticate(
    [ 'apitoken', 'basic', 'legacy-basic', 'local'],
    function apiReqUserCallback(err, user, info) {
      if (err) {
        return next(err);
      }
      if (user) {
