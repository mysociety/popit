...
module.exports.middleware = function (req,res,next) {
  ...
  req.permissions = res.locals.permissions = {};
  ...
  if ( req.user ) {
    req.popit.permissions().getPermissions( req.user._id, req.popit.instance_id(), function(err, permissions) {
      if ( err ) {
        return next(err);
      }
      req.permissions = res.locals.permissions = permissions;
      next();
    });
  } else {
    next();
...
module.exports.requirePermission = function requirePermission(permission) {
