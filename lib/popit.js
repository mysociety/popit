...
var PopIt = (function() {
    ...
    PopIt.prototype.accounts = function() {
      var name = 'Account';
      var db;
      if ( connection_cache['accounts'] ) {
        db = connection_cache['accounts'];
      } else {
        var uri = utils.mongodb_connection_string( config.MongoDB.master_name );
        connection_cache['accounts'] = mongoose.createConnection( uri );
        db = connection_cache['accounts'];
      }
      var model = null;
    ...
    // access levels
    PopIt.prototype.allow_guest_access = function () {
      return this.setting('allow_guest_access') || false;
    };
